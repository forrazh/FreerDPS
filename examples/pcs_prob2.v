...
Definition transmit (p: {prob R}) (prog : M unit) : M unit :=
prog <|| p ||> Ret tt.
...
Context {R : realType} {Fx : effect} `{StrictProvide2 Fx (@FlipEff R) client_api}
 {M : choiceEqFreerMonad Fx R _}.
Definition sharedC : contract Fx N :=
  (@flip_contract R true) -^- client_c.

Implicit Types (m : msg) (psucc : {prob R}).

Lemma freer_choice_bindDl_eq A B (p : {prob R})
    (a b : M A) (f : A -> M B) :
  (a <|| p ||> b) >>= f =
    (a >>= f) <|| p ||> (b >>= f).
Proof.
rewrite /freer_choice bindA.
congr bind.
...
Proof.
rewrite /C /lossy_send /transmit /=.
rewrite freer_choice_bindDl_eq.
