...
Module CoinFlipMod.
    Section a.
        Context {R : realType}.
Inductive CoinFlipEff : UU0 -> UU0 :=
| coin_flip_e (p : {prob R}) : CoinFlipEff bool
.

Definition coinflipf  `{Provide ix CoinFlipEff} {im : impureMonad ix} (p:{prob R}) : im bool := ( trigger  (inj_p $ coin_flip_e p)).

Definition choicef {X} `{Provide ix CoinFlipEff} {im : impureMonad ix}  (p : {prob R}) (a b : im X) := coinflipf p >>= (fun b0 => if b0 then a else b).
...
Definition open_door `{Provide2 ix DOORS CoinFlipEff} {im : impureMonad ix} (d : door) (p : {prob R}) : im unit :=
    coinflipf p >>= fun ok => when ok $
        is_open d >>= fun open =>
...
Proof.
  ...
  - inversion H2; ssubst. repeat destruct H3; ssubst.
  destruct  H4; ssubst. 
    rewrite tog_equ_1.
    inversion H1; ssubst.
    now rewrite H6.
