...
Local Notation "'pflip' p" :=
  (@flip R Fx flip_provided M p) (at level 10).
Fact freer_flip_choice (p : {prob R}) :
  (Ret true <|| p ||> Ret false : M _) ≈ pflip p.
Proof.
...
Fact flip_or_true (p q : {prob R}) :
  (Ret true <|| p ||> pflip q : M _) ≈ pflip [s_of p, q].
Proof.
rewrite -(freer_flip_choice q).
rewrite -(freer_flip_choice [s_of p, q]).
by rewrite freer_choiceA freer_choicemm.
Qed.
...
Lemma ping_pongs1 fuel :
  ping_pongs (transmit:=transmit) 1%:i01 fuel ≈ Ret GotPong.
Proof.
by elim : fuel => [|n IH];
...
Lemma ping_pong_distribution psucc :
  ping_pong (transmit:=transmit) psucc ≈
    exchange_once
      (choose:=@freer_choice R Fx flip_provided M) psucc.
Proof.
...
Lemma ping_pongs_exchanges psucc fuel :
  ping_pongs (transmit:=transmit) psucc fuel ≈
    exchanges (choose:=@freer_choice R Fx flip_provided M) psucc fuel.
Proof.
elim: fuel => [|fuel IH].
...
Lemma ping_pongs_success_stepE psucc fuel :
  ping_pongs_success (transmit:=transmit) psucc (fuel.+1) ≈
    (Ret true <|| psucc ||>
      ping_pongs_success (transmit:=transmit) psucc fuel) <|| psucc ||>
...
Theorem ping_pong_retry_success_probability psucc (fuel : nat) :
  ping_pongs_success (transmit:=transmit) (psucc%:num.~%:i01) fuel ≈
    pflip (p_exs (psucc%:num.~%:i01) fuel).
Proof.
elim: fuel => [|n].
...
have p1 : [p_of d, d] != 1%:i01.
  by rewrite p_of_rs1 (negbTE d1) andbF.
rewrite -[X in _ ≈ _ <|| _ ||> X]
  (freer_choicemm _ [q_of d, d] (pflip (p_exs d n))).
by rewrite freer_choiceA (s_of_pqK p1) (r_of_pqK p1 d0).
Qed.
